...
def turn(turn_speed):
    gyro.reset_heading()
    start_heading = gyro.get_relative_heading()

    ct.set_motors_speed(*turn_speed)

    while True:
        current_heading = gyro.get_relative_heading()
        angle_turned = absolute_angle_difference(start_heading, current_heading)

        if angle_turned < 30:
            continue
        elif ct.get_tracking() != LEFT_BLACK_RIGHT_BLACK:
            break
...
def check_direction(direction_name, turn_speed, return_speed, angle_delta=80):
    ...
    while True:
        ...
        # Ensure there has been some movement
        if angle_turned < 30:
            sleep(10)
            continue;
        elif direction_name == LEFT and tracking in (LEFT_BLACK_RIGHT_WHITE, LEFT_BLACK_RIGHT_BLACK):
            valid = True
        elif direction_name == RIGHT and tracking in (LEFT_WHITE_RIGHT_BLACK, LEFT_BLACK_RIGHT_BLACK):
            valid = True
        elif angle_turned > angle_delta:
            break

    ct.set_motors_speed(0, 0)

    # Return to original heading (forward)
    ct.set_motors_speed(*return_speed)

    while True:
        current_heading = gyro.get_relative_heading()
        angle_back = absolute_angle_difference(current_heading, start_heading)
